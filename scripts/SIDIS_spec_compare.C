...
void SIDIS_spec_compare(int runno, Int_t pisign=1, Int_t kingrp=1, Int_t ptgrp=1){
  ...
  //cointime addition:
    ...
    Double_t PgtrP;
    Double_t HgtrP;
    Double_t SHMSpartMass = 938.27208/1000.0; // proton mass in GeV/c^2 //do these need to be changed?
 ...
 //PID & kinematic cuts	
 if( !(td->GetLeaf("P.gtr.beta")->GetValue() > 0.6 && td->GetLeaf("P.gtr.beta")->GetValue() < 1.4 //proton beta cut
 	&& td->GetLeaf("H.gtr.beta")->GetValue() > 0.8 && td->GetLeaf("H.gtr.beta")->GetValue() < 1.2 //e beta cut
 	&& td->GetLeaf("H.cer.npeSum")->GetValue() > 0.0 && td->GetLeaf("P.hgcer.npeSum")->GetValue() < 0.1) //Cerenkov counter cut for c- and proton
 	&& td->GetLeaf("H.cal.etottracknorm")->GetValue() > 0.6 && td->GetLeaf("H.cal.etottracknorm")->GetValue() < 2.0 //HMS Calorimeter cut
 	&& td->GetLeaf("P.hod.2y.nhits")->GetValue() > 0 //quart-bar hit
 	...
 	&& td->GetLeaf("P.gtr.dp")->GetValue() > -10 && td->GetLeaf("P.gtr.dp")->GetValue() > 10) //p Delta cut
 	continue;
...
//coincidence time:
	...
	c2->Divide(2,1);
	c2->cd(1);
	h1PcointimeROC1->DrawNormalized();
	c2->cd(2);
	h1PcointimeROC2->DrawNormalized();
