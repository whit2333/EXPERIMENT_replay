...
void SIDIS_kine_compare(Int_t runnumber, Int_t targ=1, Int_t pisign=1, Int_t kingrp=1, Int_t ptgrp=1){
  ...
  if (pisign == 1){
   fileNameM += "sidis_pip"; //read the root file from SIMC
  }
  else if (pisign == 2){
   fileNameM += "sidis_pim"; //read the root file from SIMC
  }
  fileNameM +=kingrp;
  fileNameM += "_pt";
  fileNameM +=ptgrp;
  ...
  if (targ ==1) Double_t mtarget = 1.87561298212; //Gev/c2  mass of deuterium (check,verify)
  else if (targ == 2) Double_t mtarget = 0.93827; //Gev/c2  mass of hydrogen for run 2050
  ...
  TH1D *h_W2d = new TH1D("h_W2d","W2 (GeV2)", 150, 0, 15.0);
  TH1D *h_W2s = new TH1D("h_W2s","W2 (GeV2)", 150, 0, 15.0);
  TH1D *h_xd = new TH1D("h_xd","X_bj", 100,0, 2);   //added                                      
  TH1D *h_xs = new TH1D("h_xs","X_bj", 100,0, 2);    //added                                   
  TH1D *h_zd = new TH1D("h_zd","z_had", 100,0, 2);   //added                                     
  ...
  Double_t pkinomega, pkinQ2, pkinxbj, pEm, pPm, pbeta, hbeta, hcalepr, hcaletot, hcernpe, pcaletot, pcalepr, pcernpe;
  ...
  if (targ == 1) tt->SetBranchAddress("P.kin.secondary.emiss_nuc", &pEm);   //missing energy                                           else if (targ == 2) tt->SetBranchAddress("P.kin.secondary.emiss", &pEm);   //missing energy     
  ...
  for (int kk=0; kk<nentriesD; kk++){
    ...
    if (kk % 10000 == 0) cout << kk*100/nentriesD << "   % of data done" << endl;
     ...
     if (pbeta>0.6 && pbeta<1.4 && hbeta>0.8 && hbeta<1.2 && pcernpe<0.1 && hcernpe>0. && hcaletot >0.6 && hcaletot<2.0 && PhodStatus == 1 && HhodStatus ==1 && hdelta > -10 && hdelta < 10 && pdelta > -12 && pdelta < 12 && pkinW > 0.75 && pkinW < 1.15)     { //cuts to select the electrons and pions
	  cnts++;
  ...
  cc->SaveAs("UTIL_SIDIS/PLOTS/sidis_kine_comp.pdf");
